...
abstract class CursorDao(private val cursor: Cursor) {
    ...
    private inline fun <T> column(column: String, crossinline get: Cursor.(Int) -> T): CursorAccessor<T> {
    ...
    private inline fun <T> nullableColumn(column: String, crossinline get: Cursor.(Int) -> T): CursorAccessor<T?> {
        mutableProjection += column
...
abstract class CursorDaoCompanion<T : CursorDao> {
    ...
    fun <R> query(contentResolver: ContentResolver, order: SortOrder = SortOrder.Ascending, block: T.() -> R) =
